...
/// JS8 Normal-mode audio for a band of stations. Sample 0 is a 15 s slot
/// boundary; each station sends its frames in consecutive slots, starting
/// 0.5 s into each slot as JS8Call does. Adds Gaussian noise of RMS
/// `noise_rms` (full scale 1.0). The result is `slots` x 15 s long, where
/// `slots` is the longest station's frame count.
